...
//    { { { { { { C U R L Y B O I S } } } } } }
  ...
  val listTodaysSales = (todaysSales: ArrayBuffer[Sale]) => {
    todaysSales.foreach(sale=> {println(s"Sale " + sale.id); sale.listOfItems.foreach(stockItem => println("  " + stockItem.productName + "    = " + stockItem.salePrice)); println("Total = " + sale.totalPrice)})
  }
